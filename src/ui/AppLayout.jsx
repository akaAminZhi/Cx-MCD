...
function AppLayout() {
  ...
  const [isOrbPinned, setIsOrbPinned] = useState(false);
  const closeTimerRef = useRef(null);
  const isOrbExpanded = isOrbPinned || showOrbMenu;
        ...
        <div className="relative w-full h-full">
          <button
            ...
            className={`absolute left-0 top-[6rem] w-28 h-28 rounded-full shadow-[0_18px_50px_rgba(15,23,42,0.24)] border border-white/80 bg-[radial-gradient(circle_at_30%_30%,#fef08a_0%,#86efac_35%,#60a5fa_70%,#a78bfa_100%)] ring-4 ring-white/60 backdrop-blur-md transition-transform duration-300 ${
              isOrbExpanded ? "scale-110" : "scale-100"
            }`}
            title={isOrbPinned ? "Unpin quick menu" : "Pin quick menu"}
          />
          <div className="absolute left-0 top-[6rem] w-28 h-28 flex items-center justify-center text-white drop-shadow pointer-events-none">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              } ${isOrbExpanded ? "opacity-100 scale-100 pointer-events-auto hover:scale-110 hover:-translate-y-0.5" : "opacity-0 scale-75 pointer-events-none"}`
            }
          >
