...
function StepsTable({ children, testSteps }) {
  ...
  const numStep = testSteps?.length;
  ...
  const handleNext = () => {
    ...
    const currentRecord = {
      pfptStep,
      description: testSteps.at(pfptStep)?.description,
...
function SubdevicesSelect() {
  const { testSteps, checkboxSelect, setCheckboxSelect } =
